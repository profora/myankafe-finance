...
export default function Users(){
 ...
 const[f,setF]=useState({Username:"",DisplayName:"",Email:"",Password:""});const[assign,setAssign]=useState({user_id:"",role:"BOOKKEEPER"});
 ...
 async function create(){try{await api("/users",{method:"POST",body:JSON.stringify(f)});setMsg("User created.");setF({Username:"",DisplayName:"",Email:"",Password:""});load()}catch(e){setErr(e instanceof Error?e.message:String(e))}}
 ...
 <div className="card form"><h3>New user</h3><div className="field"><label>Username</label><input value={f.Username} onChange={e=>setF({...f,Username:e.target.value})}/></div><div className="field"><label>Display name</label><input value={f.DisplayName} onChange={e=>setF({...f,DisplayName:e.target.value})}/></div><div className="field"><label>Email</label><input value={f.Email} onChange={e=>setF({...f,Email:e.target.value})}/></div><div className="field"><label>Initial password</label><input type="password" autoComplete="new-password" value={f.Password} onChange={e=>setF({...f,Password:e.target.value})}/><span className="muted">Minimum 12 characters. The user can change it from Security after signing in.</span></div><button disabled={!f.Username||!f.DisplayName||f.Password.length<12} onClick={create}>Create user</button></div>
