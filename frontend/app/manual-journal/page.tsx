...
export default function ManualJournal(){
  ...
  useEffect(()=>{
    if(!entity||!mayPost){setAccounts([]);setLoadingRefs(false);return}
    let cancelled=false;
    setLoadingRefs(true);setError("");setMessage("");setAccounts([]);setAttachments([]);
    setDate(dateInTimeZone(entity.Timezone));setDescription("");
    setLines([{AccountPublicID:"",Debit:"",Credit:"",Description:""},{AccountPublicID:"",Debit:"",Credit:"",Description:""}]);
    api<{items:Account[]}>(`/entities/${entity.PublicID}/accounts`)
      .then(x=>{if(!cancelled)setAccounts(x.items.filter(a=>a.Postable&&a.Active))})
      .catch(e=>{if(!cancelled)setError(e instanceof Error?e.message:String(e))})
      .finally(()=>{if(!cancelled)setLoadingRefs(false)});
    return()=>{cancelled=true};
  },[entity?.PublicID,mayPost]);
  ...
  return <>
    ...
    {mayPost&&<div className="card form" aria-busy={loadingRefs}>
      ...
      {lines.map((l,i)=><div className="split-row" key={i}>
        <div className="field"><label>Account</label><select value={l.AccountPublicID} disabled={loadingRefs} onChange={e=>update(i,"AccountPublicID",e.target.value)}><option value="">{loadingRefs?"Loading accounts…":"Choose…"}</option>{accounts.map(a=><option key={a.PublicID} value={a.PublicID}>{a.Code} · {a.Name}</option>)}</select></div>
        ...
        <button type="button" className="danger" disabled={lines.length===2} onClick={()=>setLines(xs=>xs.filter((_,n)=>n!==i))}>×</button>
      </div>)}
      <div className="actions"><button type="button" className="secondary" disabled={loadingRefs} onClick={()=>setLines(xs=>[...xs,{AccountPublicID:"",Debit:"",Credit:"",Description:""}])}>+ Line</button><span style={{marginLeft:"auto"}}>Debits <strong>{debit.toLocaleString()}</strong> · Credits <strong>{credit.toLocaleString()}</strong></span></div>
      ...
      <button type="button" disabled={loadingRefs||busy||!description||debit<=0||debit!==credit||lines.some(x=>!x.AccountPublicID)} onClick={post}>{busy?"Posting…":"Post journal"}</button>
    </div>}
  </>;
}
