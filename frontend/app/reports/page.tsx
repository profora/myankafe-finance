...
export default function Reports(){
  ...
  return <>
    ...
    <div className="card form" style={{marginBottom:16}}>
      ...
      <div className="actions"><button type="button" disabled={loading||!from||!to||from>to} onClick={load}>{loading?"Refreshing…":"Apply report range"}</button><span className="muted">P&L, ledger and cash movement use the range. Balance Sheet and Trial Balance are through the end date.</span></div>
    </div>

    <div className="grid" style={{gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,420px),1fr))"}}>
      <div><div className="report-title"><h3>Profit & Loss</h3><button type="button" className="secondary compact" disabled={loading||pl.length===0} onClick={exportPL}>Export CSV</button></div><div className="table-wrap" aria-busy={loading}><table><thead><tr><th>Account</th><th>Type</th><th>Amount</th></tr></thead><tbody>{pl.map((x,n)=><tr key={x.id??n}><td>{x.id?<Link className="table-link" href={`/accounts/${x.id}/ledger`}>{x.code} · {x.name}</Link>:<>{x.code} · {x.name}</>}</td><td>{x.type}</td><td>{Number(x.amount||0).toLocaleString()}</td></tr>)}<TableStateRows loading={loading&&pl.length===0} empty={!loading&&pl.length===0} columns={3} emptyText="No posted income or expense activity in this range."/></tbody></table></div></div>
      <div><div className="report-title"><h3>Balance Sheet</h3><button type="button" className="secondary compact" disabled={loading||(bs.length===0&&Number(earnings)===0)} onClick={exportBS}>Export CSV</button></div><div className="table-wrap" aria-busy={loading}><table><thead><tr><th>Account</th><th>Type</th><th>Balance</th></tr></thead><tbody>{bs.map((x,n)=><tr key={x.id??n}><td>{x.id?<Link className="table-link" href={`/accounts/${x.id}/ledger`}>{x.code} · {x.name}</Link>:<>{x.code} · {x.name}</>}</td><td>{x.type}</td><td>{Number(x.balance||0).toLocaleString()}</td></tr>)}{!loading&&(bs.length>0||Number(earnings)!==0)&&<tr><td><strong>Current earnings</strong></td><td>EQUITY</td><td><strong>{Number(earnings).toLocaleString()}</strong></td></tr>}<TableStateRows loading={loading&&bs.length===0} empty={!loading&&bs.length===0&&Number(earnings)===0} columns={3} emptyText="No posted balance-sheet activity through this date."/></tbody></table></div></div>
    ...
    <div className="page-head" style={{marginTop:28}}><div><h1 style={{fontSize:20}}>General Ledger</h1><p>Most recent 200 lines shown; CSV export fetches up to 2,000 lines for the selected range.</p></div><button type="button" className="secondary compact" disabled={loading||gl.length===0} onClick={exportGL}>Export CSV</button></div>
    <div className="table-wrap" aria-busy={loading}><table><thead><tr><th>Date</th><th>Account</th><th>Description</th><th>Debit</th><th>Credit</th></tr></thead><tbody>{gl.map((x,n)=><tr key={x.journal_id+n}><td>{x.date}</td><td><Link className="table-link" href={`/accounts/${x.account_id}/ledger`}>{x.account_code} · {x.account_name}</Link></td><td>{x.description}</td><td>{Number(x.debit).toLocaleString()}</td><td>{Number(x.credit).toLocaleString()}</td></tr>)}<TableStateRows loading={loading&&gl.length===0} empty={!loading&&gl.length===0} columns={5} emptyText="No general-ledger lines in this range."/></tbody></table></div>
  </>;
}
