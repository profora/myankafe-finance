...
export default function Transactions() {
  ...
  return <>
    ...
    <div className="table-wrap">{items.length?<table><thead><tr><th>Date</th><th>Type</th><th>Description</th><th>Status</th><th>Amount</th><th></th></tr></thead><tbody>{items.map(t=><tr key={t.PublicID}><td>{t.Date}</td><td>{t.Type}</td><td><Link className="table-link" href={`/transactions/${t.PublicID}`}>{t.Description}</Link></td><td><span className={`badge ${t.Status}`}>{t.Status}</span></td><td>{Number(t.Total).toLocaleString()} {t.Currency}</td><td>{t.Status==="DRAFT"&&<button disabled={busy===t.PublicID} onClick={()=>post(t.PublicID)}>{busy===t.PublicID?"Posting…":"Post"}</button>}{t.Status==="POSTED"&&<button className="danger" disabled={busy===t.PublicID} onClick={()=>{setReverseID(t.PublicID);setReverseReason("");setReverseDate(dateInTimeZone(entity?.Timezone??"Asia/Yangon"))}}>Reverse</button>}</td></tr>)}</tbody></table>:<div className="empty">No transactions yet.</div>}</div>
    <ConfirmDialog
