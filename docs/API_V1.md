...
- `GET /entities`
- `PUT /entities/{entity}/accounting-start-date` sets `accounting_start_date`. OWNER, ADMIN, and ACCOUNTANT may set it only before an opening-balance set or any posted/reversed journal exists. While it is null, setup is allowed and accounting transactions are rejected.
- `GET /entities/{entity}/opening-balances`, `POST /entities/{entity}/opening-balances/preview`, and `PUT /entities/{entity}/opening-balances` maintain the current desired opening position. OWNER and ACCOUNTANT may edit while the start date is outside the locked period. The first save posts an immutable `OPENING_BALANCE` journal, using `3990` (`OPENING_BALANCE_EQUITY`) only when the entered balance sheet does not balance. Later saves post immutable `OPENING_BALANCE_ADJUSTMENT` journals for the delta, using `3980` (`OPENING_BALANCE_ADJUSTMENT`) only when that delta does not balance. An unchanged save creates no journal. These transaction types cannot be reversed from the generic reversal endpoint.
- `POST /entities` — requires OWNER on an existing entity, or `platform_owner` on the signed-in user. Optional `Template` selects `MYANKAFE_BUSINESS`, `ROYAL_MASTERPIECE`, `PERSONAL`, or `GENERAL_BUSINESS`. The chart of accounts is seeded in the same transaction. `platform_owner` is the global ownership flag: every active platform owner is automatically OWNER of every entity, including a newly created one, in the same transaction. With zero entities there is no role row yet, and the platform owner is still treated as OWNER. Creating or granting an entity OWNER role does not set `platform_owner`. Ordinary user creation leaves it false. An active platform owner cannot be downgraded or removed from an entity. `GET /auth/me` returns `platform_owner`.
- `GET /users`
- `POST /users`
