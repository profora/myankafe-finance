...
func main() {
	...
	if credentialErr != nil && !errors.Is(credentialErr, pgx.ErrNoRows) {
		log.Fatal(credentialErr)
	}
	if cfg.AuthMode == "password" && password == "" && errors.Is(credentialErr, pgx.ErrNoRows) {
		log.Fatal("BOOTSTRAP_PASSWORD is required when AUTH_MODE=password and the owner has no password credential")
	}
	if password != "" {
...
SET password_hash=EXCLUDED.password_hash,
	...
	if strings.EqualFold(getenv("BOOTSTRAP_ENTITIES", "true"), "true") {
		...
		for _, e := range []spec{
			...
			roleLinkID, err := ids.UUIDv7()
			if err != nil {
				log.Fatal(err)
			}
			if _, err = tx.Exec(ctx, `
INSERT INTO user_entity_roles(id,user_id,entity_id,role_id,granted_by)
VALUES($1,$2,$3,'00000000-0000-7000-8000-000000000001',$2)
ON CONFLICT DO NOTHING`, roleLinkID, userID, entityID); err != nil {
				log.Fatal(err)
			}
		}
	}
