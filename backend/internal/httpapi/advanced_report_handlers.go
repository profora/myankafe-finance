...
func reportRange(r *http.Request, entity postgres.Entity)(time.Time,time.Time,error){
	now:=time.Now().UTC()
	from,err:=parseDate(r.URL.Query().Get("from"),fiscalYearStart(entity,now));if err!=nil{return time.Time{},time.Time{},err}
...
func (s *Server) generalLedger(w http.ResponseWriter,r *http.Request){
	a:=getAccess(r);from,to,err:=reportRange(r,a.Entity);if err!=nil{fail(w,400,err);return};limit,_:=strconv.Atoi(r.URL.Query().Get("limit"))
...
func (s *Server) cashMovement(w http.ResponseWriter,r *http.Request){
	a:=getAccess(r);from,to,err:=reportRange(r,a.Entity);if err!=nil{fail(w,400,err);return}
