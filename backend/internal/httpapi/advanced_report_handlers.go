...
func reportRange(r *http.Request, entity postgres.Entity) (time.Time, time.Time, error) {
	now := time.Now().UTC()
	from, err := parseDate(r.URL.Query().Get("from"), fiscalYearStart(entity, now))
	if err != nil {
...
func (s *Server) balanceSheet(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.Store.BalanceSheet(r.Context(), a.Entity.ID, through)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) generalLedger(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.Store.GeneralLedger(r.Context(), a.Entity.ID, from, to, limit)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) accountLedger(w http.ResponseWriter, r *http.Request) {
	...
	if account == "" {
		fail(w, 400, errString("account_id is required"))
		return
	}
	v, err := s.Store.AccountLedger(r.Context(), a.Entity.ID, account, from, to, limit)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) cashMovement(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.Store.CashMovement(r.Context(), a.Entity.ID, from, to)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) interEntityBalances(w http.ResponseWriter, r *http.Request) {
	...
	v, err := s.Store.InterEntityBalances(r.Context(), a.Entity.ID, through)
	if err != nil {
		fail(w, 500, err)
		return
	}
