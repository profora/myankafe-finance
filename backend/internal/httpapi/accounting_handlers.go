...
func (s *Server) profitLoss(w http.ResponseWriter, r *http.Request) {
	a := getAccess(r)
	now := time.Now().UTC()
	from, err := parseDate(r.URL.Query().Get("from"), fiscalYearStart(a.Entity, now))
	if err != nil {
		fail(w, 400, err)
		return
	}
	...
	v, err := s.Store.ProfitLoss(r.Context(), a.Entity.ID, from, to)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) trialBalance(w http.ResponseWriter, r *http.Request) {
	a := getAccess(r)
	through, err := parseDate(r.URL.Query().Get("through"), time.Now().UTC())
	if err != nil {
		fail(w, 400, err)
		return
	}
	v, err := s.Store.TrialBalance(r.Context(), a.Entity.ID, through)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) listExchangeRates(w http.ResponseWriter, r *http.Request) {
	a := getAccess(r)
	v, err := s.Store.ListExchangeRates(r.Context(), a.Entity.ID)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) createExchangeRate(w http.ResponseWriter, r *http.Request) {
	a := getAccess(r)
	if !requireRole(w, canConfigureAccounting(a.Role), "exchange-rate configuration requires OWNER, ADMIN, or ACCOUNTANT") {
		return
	}
...
func (s *Server) manualJournal(w http.ResponseWriter, r *http.Request) {
	...
	if a.Role != "OWNER" && a.Role != "ACCOUNTANT" {
		fail(w, 403, errors.New("manual journal requires OWNER or ACCOUNTANT"))
		return
	}
	var in postgres.ManualJournalInput
	if err := json.NewDecoder(r.Body).Decode(&in); err != nil {
		fail(w, 400, err)
		return
	}
	v, err := s.Store.PostManualJournal(r.Context(), a.User, a.Entity, in)
	if err != nil {
		fail(w, 400, err)
		return
	}
