...
func (s *Server) exportTransactionsCSV(w http.ResponseWriter,r *http.Request){
	...
	status:=q.Get("status")
	switch status{case "","DRAFT","POSTED","VOIDED":default:fail(w,400,errString("invalid status filter"));return}
	typ:=q.Get("type")
	switch typ{
	case "","INCOME","EXPENSE","ACCOUNT_TRANSFER","INTER_ENTITY","MANUAL_JOURNAL","ADJUSTMENT","REVERSAL":
	default:fail(w,400,errString("invalid type filter"));return
	}
	...
	for {
		...
		for _,item:=range result.Items{
			contact:=""
			if item.ContactName!=nil{contact=*item.ContactName}
			financial:=""
			if item.FinancialAccount!=nil{financial=*item.FinancialAccount}
			_ = writer.Write([]string{
		...
		if len(result.Items)==0||!result.HasMore{break}
...
func safeCSVFilename(v string)string{
	...
	if !strings.HasSuffix(strings.ToLower(out),".csv"){out+=".csv"}
	return out
}
