...
func (s *Server) login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fail(w, 500, errors.New("could not create session"))
		return
	}
...
func (s *Server) changePassword(w http.ResponseWriter, r *http.Request) {
	...
	currentHash, err := s.Store.CredentialByUserID(r.Context(), u.ID)
	if err != nil {
		fail(w, 500, errors.New("could not load credentials"))
		return
	}
	match, _, err := auth.VerifyPassword(in.CurrentPassword, currentHash)
	if err != nil || !match {
		fail(w, 401, errors.New("current password is incorrect"))
		return
	}
	...
	if err != nil {
		fail(w, 500, errors.New("could not change password"))
		return
	}
	s.setSessionCookie(w, rawToken, session.ExpiresAt)
...
func (s *Server) listSessions(w http.ResponseWriter, r *http.Request) {
	...
	items, err := s.Store.ListActiveSessions(r.Context(), u.ID)
	if err != nil {
		fail(w, 500, err)
		return
	}
...
func (s *Server) revokeOtherSessions(w http.ResponseWriter, r *http.Request) {
	...
	if p.SessionID == "" {
		fail(w, 400, errors.New("current session cannot be identified in development auth mode"))
		return
	}
	count, err := s.Store.RevokeOtherSessions(r.Context(), u.ID, p.SessionID)
	if err != nil {
		fail(w, 500, err)
		return
	}
