...
func (s *Server) uploadTransactionAttachments(w http.ResponseWriter, r *http.Request) {
	...
	if len(files) == 0 {
		fail(w, 400, errors.New("at least one file is required"))
		return
	}
	if len(files) > maxAttachmentsPerUpload {
		fail(w, 400, fmt.Errorf("at most %d files may be uploaded at once", maxAttachmentsPerUpload))
		return
	}
	...
	for _, fh := range files {
		...
		if len(body) == 0 {
			cleanup()
			fail(w, 400, fmt.Errorf("%s is empty", fh.Filename))
			return
		}
...
func (s *Server) reorderTransactionAttachments(w http.ResponseWriter, r *http.Request) {
	...
	if len(in.AttachmentIDs) == 0 {
		fail(w, 400, errors.New("attachment_ids is required"))
		return
	}
