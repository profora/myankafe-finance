...
func (s *Server) createInterEntityExpense(w http.ResponseWriter, r *http.Request) {
	...
	initEntity, initRole, err := s.Store.ResolveEntityAccess(r.Context(), u.ID, body.InitiatingEntityID)
	if err != nil {
		fail(w, 403, err)
		return
	}
	cpEntity, cpRole, err := s.Store.ResolveEntityAccess(r.Context(), u.ID, body.CounterpartyEntityID)
	if err != nil {
		fail(w, 403, err)
		return
	}
	if !canOperateLedger(initRole) || !canOperateLedger(cpRole) {
		fail(w, 403, errors.New("ledger operation access required on both entities"))
		return
	}
