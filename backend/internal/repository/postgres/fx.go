...
LIMIT 200`,entityID)
	...
	for rows.Next(){
		...
		var used bool
		if err:=rows.Scan(&id,&date,&from,&to,&rate,&source,&ref,&created,&used);err!=nil{return nil,err}
...
func validateExchangeRateInput(date,from,to,rate,source string) error {
	if _,err:=time.Parse("2006-01-02",date);err!=nil{return fmt.Errorf("invalid rate date")}
	from=strings.ToUpper(strings.TrimSpace(from))
	to=strings.ToUpper(strings.TrimSpace(to))
	if len(from)!=3||len(to)!=3||from==to{return fmt.Errorf("invalid currency pair")}
	r,ok:=new(big.Rat).SetString(strings.TrimSpace(rate))
	if !ok||r.Sign()<=0{return fmt.Errorf("rate must be greater than zero")}
	switch source{
	case "MANUAL","INTEGRATION","SYSTEM":
	default:return fmt.Errorf("invalid exchange rate source")
	}
	return nil
}
...
func (s *Store) CreateExchangeRate(ctx context.Context,user User,e Entity,date,from,to,rate,source,reference string)(map[string]any,error){
	from=strings.ToUpper(strings.TrimSpace(from));to=strings.ToUpper(strings.TrimSpace(to))
	if source==""{source="MANUAL"}
	source=strings.ToUpper(strings.TrimSpace(source))
	if err:=validateExchangeRateInput(date,from,to,rate,source);err!=nil{return nil,err}
...
func (s *Store) UpdateExchangeRate(ctx context.Context,user User,e Entity,publicID string,in UpdateExchangeRateInput)(map[string]any,error){
	...
	if err:=validateExchangeRateInput(in.RateDate,in.FromCurrency,in.ToCurrency,in.Rate,"MANUAL");err!=nil{return nil,err}
...
FOR UPDATE`,e.ID,publicID).Scan(&source);err!=nil{return nil,err}
	if source!="MANUAL"{return nil,fmt.Errorf("only MANUAL exchange rates can be edited")}
...
FROM exchange_rates
WHERE entity_id=$1 AND public_id=$2
FOR UPDATE`,e.ID,publicID).Scan(&source);err!=nil{return err}
	if source!="MANUAL"{return fmt.Errorf("only MANUAL exchange rates can be deleted")}

	if _,err:=tx.Exec(ctx,`DELETE FROM exchange_rates WHERE entity_id=$1 AND public_id=$2`,e.ID,publicID);err!=nil{return err}
	if err:=insertAuditTx(ctx,tx,user,e,"EXCHANGE_RATE_DELETE","EXCHANGE_RATE",publicID,map[string]any{"deleted":true});err!=nil{return err}
