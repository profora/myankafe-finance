...
ORDER BY ta.display_order,ta.created_at,ta.public_id`, entityID, transactionPublicID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.PublicID, &a.OriginalFilename, &a.MimeType, &a.SizeBytes, &a.DisplayOrder, &a.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, a)
	}
...
FOR UPDATE`, e.ID, transactionPublicID).Scan(&transactionID); err != nil {
		return nil, err
	}
...
WHERE transaction_id=$1 AND deleted_at IS NULL`, transactionID).Scan(&existing); err != nil {
		return nil, err
	}
	if existing+len(input) > 50 {
		return nil, fmt.Errorf("a transaction can have at most 50 active attachments")
	}
...
WHERE transaction_id=$1 AND deleted_at IS NULL`, transactionID).Scan(&nextOrder); err != nil {
		return nil, err
	}
...
INSERT INTO transaction_attachments(
 ...
 uploaded_by,display_order,sha256_hex
		...
		if err := tx.QueryRow(ctx, `SELECT created_at FROM transaction_attachments WHERE id=$1`, internalID).Scan(&created); err != nil {
			return nil, err
		}
	...
	if err := insertAuditTx(ctx, tx, user, e, "TRANSACTION_ATTACHMENTS_UPLOAD", "TRANSACTION", transactionPublicID, map[string]any{"count": len(out)}); err != nil {
		return nil, err
	}
...
WHERE transaction_id=(SELECT id FROM transactions WHERE entity_id=$1 AND public_id=$2)
  AND public_id=$3 AND deleted_at IS NULL`,
		e.ID, transactionPublicID, attachmentPublicID, user.ID); err != nil {
		return AttachmentObject{}, err
	}
...
SELECT id::text FROM transactions WHERE entity_id=$1 AND public_id=$2 FOR UPDATE`,
		e.ID, transactionPublicID).Scan(&transactionID); err != nil {
		return err
	}
...
FOR UPDATE`, transactionID)
	...
	if len(active) != len(attachmentPublicIDs) {
		return fmt.Errorf("reorder must include every active attachment exactly once")
	}
	...
	for _, id := range attachmentPublicIDs {
		if !activeSet[id] || seen[id] {
			return fmt.Errorf("invalid or duplicate attachment in reorder")
		}
		seen[id] = true
	}
...
WHERE transaction_id=$1 AND deleted_at IS NULL`, transactionID); err != nil {
		return err
	}
...
WHERE transaction_id=$1 AND public_id=$2 AND deleted_at IS NULL`,
			transactionID, id, order); err != nil {
			return err
		}
	}
