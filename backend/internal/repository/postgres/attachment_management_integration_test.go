...
func TestAttachmentReorderAndSoftDelete(t *testing.T) {
	...
	if len(items) != 2 || items[0].PublicID != a || items[1].PublicID != b {
		t.Fatalf("initial items=%+v", items)
	}

	if err := s.ReorderTransactionAttachments(ctx, user, entity, tx.PublicID, []string{b, a}); err != nil {
		t.Fatal(err)
	}
	...
	if len(items) != 2 || items[0].PublicID != b || items[1].PublicID != a {
		t.Fatalf("reordered items=%+v", items)
	}

	deleted, err := s.SoftDeleteTransactionAttachment(ctx, user, entity, tx.PublicID, b)
	if err != nil {
		t.Fatal(err)
	}
	if deleted.PublicID != b {
	...
	if len(items) != 1 || items[0].PublicID != a {
		t.Fatalf("remaining items=%+v", items)
	}

	var deletedAt any
	if err := s.Pool.QueryRow(ctx, `SELECT deleted_at FROM transaction_attachments WHERE public_id=$1`, b).Scan(&deletedAt); err != nil {
		t.Fatal(err)
	}
	if deletedAt == nil {
		t.Fatal("soft-deleted attachment must retain deleted_at")
	}
}
