...
INSERT INTO entities(
 id,public_id,code,name,entity_type,functional_currency_code,timezone,created_by
		...
		entityID,entityPublic,"ACCESS_"+entityPublic,actor.ID);err!=nil{t.Fatal(err)}
...
VALUES($1,$2,$3,'00000000-0000-7000-8000-000000000001',$4)`,
	...
	if err:=s.RevokeUserEntityAccess(ctx,actor,target.PublicID,entity);err!=nil{t.Fatal(err)}

	var activeRoles int
	if err:=s.Pool.QueryRow(ctx,`
SELECT count(*) FROM user_entity_roles
WHERE user_id=$1 AND entity_id=$2 AND revoked_at IS NULL`,target.ID,entityID).Scan(&activeRoles);err!=nil{t.Fatal(err)}
	if activeRoles!=0{t.Fatalf("active role rows=%d want 0",activeRoles)}

	var auditCount int
	if err:=s.Pool.QueryRow(ctx,`
SELECT count(*) FROM audit_events
WHERE entity_id=$1 AND action='USER_ENTITY_ACCESS_REVOKE' AND resource_public_id=$2`,
		entityID,target.PublicID).Scan(&auditCount);err!=nil{t.Fatal(err)}
	if auditCount!=1{t.Fatalf("access revoke audit count=%d want 1",auditCount)}
}
