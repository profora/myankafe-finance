...
FROM users u WHERE id=$1`, target.ID).Scan(&status, &activeSessions); err != nil {
		t.Fatal(err)
	}
	if status != "DISABLED" {
		t.Fatalf("status=%s want DISABLED", status)
	}
	if activeSessions != 0 {
		t.Fatalf("active sessions=%d want 0", activeSessions)
	}

	if err := s.SetUserStatus(ctx, actor, target.PublicID, "ACTIVE"); err != nil {
		t.Fatal(err)
	}
	if err := s.Pool.QueryRow(ctx, `SELECT status FROM users WHERE id=$1`, target.ID).Scan(&status); err != nil {
		t.Fatal(err)
	}
	if status != "ACTIVE" {
...
WHERE actor_user_id=$1 AND resource_public_id=$2 AND action='USER_STATUS_CHANGE' AND outcome='SUCCESS'`,
		actor.ID, target.PublicID).Scan(&auditCount); err != nil {
		t.Fatal(err)
	}
	if auditCount != 2 {
		t.Fatalf("status audit count=%d want 2", auditCount)
	}
}
...
func TestUserCannotDisableSelf(t *testing.T) {
	...
	var status string
	if err := s.Pool.QueryRow(ctx, `SELECT status FROM users WHERE id=$1`, actor.ID).Scan(&status); err != nil {
		t.Fatal(err)
	}
	if status != "ACTIVE" {
		t.Fatalf("self-disable mutated status=%s", status)
	}
}
...
INSERT INTO entities(
 id,public_id,code,name,entity_type,functional_currency_code,timezone,created_by
		...
		entityID, entityPublic, "SOLE_"+entityPublic, actor.ID); err != nil {
		t.Fatal(err)
	}
