...
func (s *Store) ListAccounts(ctx context.Context,entityID string)([]Account,error){
  ...
  if err!=nil{return nil,err}; defer rows.Close(); out:=[]Account{}
  for rows.Next(){var a Account;if err:=rows.Scan(&a.PublicID,&a.Code,&a.Name,&a.Type,&a.Subtype,&a.Postable,&a.Active);err!=nil{return nil,err};out=append(out,a)}
  return out,rows.Err()
}
...
RETURNING public_id::text,code,name,account_type,account_subtype,is_postable,active`,id,pub,e.ID,in.Code,in.Name,parent,in.Type,in.Subtype,in.Postable,user.ID).Scan(&a.PublicID,&a.Code,&a.Name,&a.Type,&a.Subtype,&a.Postable,&a.Active)
  if err==nil { _=s.Audit(ctx,user,&e,"COA_CREATE","ACCOUNT",&pub,"SUCCESS",map[string]any{"code":in.Code,"name":in.Name}) }
  return a,err
}
...
func (s *Store) Audit(ctx context.Context,user User,e *Entity,action,resource string,pub *string,outcome string,after map[string]any)error{
  id,_:=ids.UUIDv7(); apub,_:=ids.ULID(); var eid any
  if e!=nil{eid=e.ID}
...
func (s *Store) EnsureOpenDateTx(ctx context.Context,tx pgx.Tx,entityID,date string)error{
  ...
  if locked!=nil{
    d,err:=time.Parse("2006-01-02",date); if err!=nil{return err}
    if !d.After(*locked){return fmt.Errorf("accounting period is locked through %s",locked.Format("2006-01-02"))}
  }
  return nil
}
