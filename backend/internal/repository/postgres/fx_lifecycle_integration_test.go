...
func TestUnusedManualExchangeRateCanBeCorrectedAndDeleted(t *testing.T) {
	...
	if updated["rate"] != "4100" {
		t.Fatalf("updated rate=%v", updated["rate"])
	}

	if err := s.DeleteExchangeRate(ctx, user, entity, id); err != nil {
		t.Fatal(err)
	}
	var count int
	if err := s.Pool.QueryRow(ctx, `SELECT count(*) FROM exchange_rates WHERE public_id=$1`, id).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 0 {
		t.Fatalf("remaining rate rows=%d want 0", count)
	}
}
...
VALUES($1,$2,$3,$4,$5,'USD Bank','BANK','USD',$6)`,
		faID, faPublic, entity.ID, assetID, "FA_"+faPublic, user.ID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.PostTransaction(ctx, user, entity, tx.PublicID); err != nil {
		t.Fatal(err)
	}

	rates, err := s.ListExchangeRates(ctx, entity.ID)
	if err != nil {
		t.Fatal(err)
	}
	var used bool
	...
	if !used {
		t.Fatal("posted rate should be marked used")
	}
