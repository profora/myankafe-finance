...
func TestEntitySettingsUpdatePreservesAccountingIdentity(t *testing.T) {
	...
	if updated.Name != "Renamed Entity" {
		t.Fatalf("name=%q", updated.Name)
	}
	if updated.Timezone != "Asia/Singapore" {
		t.Fatalf("timezone=%q", updated.Timezone)
	}
	if updated.FiscalMonth != 4 || updated.FiscalDay != 1 {
		t.Fatalf("fiscal=%d/%d", updated.FiscalMonth, updated.FiscalDay)
	}
...
WHERE entity_id=$1 AND action='ENTITY_SETTINGS_UPDATE' AND resource_public_id=$2`,
		entity.ID, entity.PublicID).Scan(&auditCount); err != nil {
		t.Fatal(err)
	}
	if auditCount != 1 {
		t.Fatalf("entity settings audit count=%d want 1", auditCount)
	}
}
