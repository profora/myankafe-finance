...
func (s *Store) ListTransactions(ctx context.Context,entityID string)([]Transaction,error){
  rows,err:=s.Pool.Query(ctx,`SELECT public_id::text,transaction_type,status,transaction_date::text,description,currency_code,total_amount::text FROM transactions WHERE entity_id=$1 ORDER BY transaction_date DESC,created_at DESC LIMIT 200`,entityID)
  if err!=nil{return nil,err};defer rows.Close();out:=[]Transaction{}
  for rows.Next(){var t Transaction;if err:=rows.Scan(&t.PublicID,&t.Type,&t.Status,&t.Date,&t.Description,&t.Currency,&t.Total);err!=nil{return nil,err};out=append(out,t)}
  return out,rows.Err()
}
