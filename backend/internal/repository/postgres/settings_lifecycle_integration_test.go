...
func TestInactiveContactPreservesHistoryButCannotBeReused(t *testing.T) {
	...
	contactID, ok := created["id"].(string)
	if !ok || contactID == "" {
		t.Fatalf("contact id=%v", created["id"])
	}
...
WHERE t.public_id=$1`, tx.PublicID).Scan(&retained); err != nil {
		t.Fatal(err)
	}
	if retained != contactID {
		t.Fatalf("historical contact=%s want %s", retained, contactID)
	}
...
func TestInactiveFinancialAccountPreservesHistoryButCannotBeReused(t *testing.T) {
	...
	if updated.Active {
		t.Fatal("expected financial account inactive")
	}
	if updated.Name != "Archived Cash" {
...
WHERE fa.public_id=$1`, financialAccount).Scan(&historyCount); err != nil {
		t.Fatal(err)
	}
	if historyCount == 0 {
		t.Fatal("expected historical journal line to retain financial account")
	}
