...
func (s *Store) UpdateContact(ctx context.Context,user User,e Entity,publicID string,in UpdateContactInput)(map[string]any,error){
	...
	default:return nil,fmt.Errorf("invalid contact type")
	}
	in.DisplayName=strings.TrimSpace(in.DisplayName)
	if in.DisplayName==""{return nil,fmt.Errorf("display name is required")}
...
FOR UPDATE`,e.ID,publicID).Scan(&id);err!=nil{return nil,err}
...
WHERE id=$1 AND entity_id=$2`,
		id,e.ID,in.Type,in.DisplayName,strings.TrimSpace(in.Phone),strings.TrimSpace(in.Email),strings.TrimSpace(in.Notes),in.Active);err!=nil{return nil,err}
...
func (s *Store) UpdateFinancialAccount(ctx context.Context,user User,e Entity,publicID string,in UpdateFinancialAccountInput)(FinancialAccount,error){
	...
	if in.Name==""{return FinancialAccount{},fmt.Errorf("name is required")}

	tx,err:=s.Pool.Begin(ctx);if err!=nil{return FinancialAccount{},err}
	defer tx.Rollback(ctx)
...
WHERE entity_id=$1 AND public_id=$2
FOR UPDATE`,e.ID,publicID).Scan(&id);err!=nil{return FinancialAccount{},err}
...
SET name=$3,
    ...
    updated_at=now()
WHERE id=$1 AND entity_id=$2`,
		id,e.ID,in.Name,strings.TrimSpace(in.Institution),strings.TrimSpace(in.Reference),in.Active);err!=nil{return FinancialAccount{},err}
...
WHERE fa.id=$1`,id).
	...
	if err:=insertAuditTx(ctx,tx,user,e,"FINANCIAL_ACCOUNT_UPDATE","FINANCIAL_ACCOUNT",publicID,map[string]any{
		"name":out.Name,
		"active":out.Active,
	});err!=nil{return FinancialAccount{},err}
