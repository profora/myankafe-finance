...
func ratFromDecimal(v string) (*big.Rat,error) {
	r:=new(big.Rat)
	if _,ok:=r.SetString(v);!ok{return nil,fmt.Errorf("invalid decimal %q",v)}
	return r,nil
}

func (s *Store) reportingRate(ctx context.Context, entityID, from, to string, asOf time.Time)(*big.Rat,error){
	if from==to{return new(big.Rat).SetInt64(1),nil}
	var rate string
...
ORDER BY rate_date DESC,created_at DESC LIMIT 1`,entityID,from,to,asOf).Scan(&rate)
	if err!=nil{return nil,fmt.Errorf("reporting rate %s -> %s missing for entity: %w",from,to,err)}
...
func (s *Store) CombinedDashboard(ctx context.Context,userID,reportingCurrency string,from,to time.Time)(CombinedDashboard,error){
	if reportingCurrency==""{reportingCurrency="MMK"}
	entities,err:=s.ListEntities(ctx,userID);if err!=nil{return CombinedDashboard{},err}
	...
	for _,e:=range entities{
		d,err:=s.Dashboard(ctx,e.ID,from,to);if err!=nil{return CombinedDashboard{},err}
		rate,err:=s.reportingRate(ctx,e.ID,e.FunctionalCurrency,reportingCurrency,to);if err!=nil{return CombinedDashboard{},err}
		income,_:=ratFromDecimal(d.Income);expense,_:=ratFromDecimal(d.Expenses);profit,_:=ratFromDecimal(d.NetProfit)
