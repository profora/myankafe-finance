...
func (s *Store) ListContacts(ctx context.Context, entityID string) ([]map[string]any, error) {
	...
	for rows.Next(){var id,typ,name string;var phone,email,notes *string;var active bool;if err:=rows.Scan(&id,&typ,&name,&phone,&email,&notes,&active);err!=nil{return nil,err};out=append(out,map[string]any{"id":id,"contact_type":typ,"display_name":name,"phone":phone,"email":email,"notes":notes,"active":active})}
	return out,rows.Err()
}
