...
ORDER BY a.account_type,a.code`,entityID,through)
	...
	for rows.Next(){var id,code,name,typ,balance string;if err:=rows.Scan(&id,&code,&name,&typ,&balance);err!=nil{return nil,err};items=append(items,map[string]any{"id":id,"code":code,"name":name,"type":typ,"balance":balance})}
...
LIMIT $4`,entityID,from,to,limit)
	...
	out:=[]map[string]any{}
	for rows.Next(){
		var journal,date,jdesc,account,code,name,ldesc,debit,credit,currency string;var line int;var tx *string
		if err:=rows.Scan(&journal,&date,&jdesc,&account,&code,&name,&line,&ldesc,&debit,&credit,&currency,&tx);err!=nil{return nil,err}
...
func (s *Store) AccountLedger(ctx context.Context,entityID,accountPublicID string,from,to time.Time,limit int)([]map[string]any,error){
	if limit<=0||limit>2000{limit=500}
	rows,err:=s.Pool.Query(ctx,`
SELECT je.journal_date::text,je.public_id::text,t.public_id::text,
       je.description,COALESCE(jl.description,''),
...
LIMIT $5`,entityID,accountPublicID,from,to,limit)
	if err!=nil{return nil,err};defer rows.Close()
	out:=[]map[string]any{}
	for rows.Next(){var date,journal,jdesc,ldesc,debit,credit,balance,currency string;var tx *string;if err:=rows.Scan(&date,&journal,&tx,&jdesc,&ldesc,&debit,&credit,&balance,&currency);err!=nil{return nil,err};out=append(out,map[string]any{"date":date,"journal_id":journal,"transaction_id":tx,"journal_description":jdesc,"description":ldesc,"debit":debit,"credit":credit,"running_balance":balance,"currency":currency})}
	return out,rows.Err()
}
...
ORDER BY je.journal_date,fa.name`,entityID,from,to)
	if err!=nil{return nil,err};defer rows.Close()
	out:=[]map[string]any{}
	for rows.Next(){var date,id,name,currency,movement string;if err:=rows.Scan(&date,&id,&name,&currency,&movement);err!=nil{return nil,err};out=append(out,map[string]any{"date":date,"financial_account_id":id,"name":name,"currency":currency,"movement":movement})}
	return out,rows.Err()
}
...
ORDER BY ce.name`,entityID,through)
	if err!=nil{return nil,err};defer rows.Close()
	out:=[]map[string]any{}
	for rows.Next(){var cp,cpName,df,dfCode,dfName,dt,dtCode,dtName,dueFrom,dueTo string;if err:=rows.Scan(&cp,&cpName,&df,&dfCode,&dfName,&dt,&dtCode,&dtName,&dueFrom,&dueTo);err!=nil{return nil,err};out=append(out,map[string]any{"counterparty_entity_id":cp,"counterparty_name":cpName,"due_from_account":map[string]any{"id":df,"code":dfCode,"name":dfName},"due_to_account":map[string]any{"id":dt,"code":dtCode,"name":dtName},"due_from":dueFrom,"due_to":dueTo})}
	return out,rows.Err()
}
